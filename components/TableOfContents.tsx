...
export function TableOfContents({ content, className, isMobile = false }: TableOfContentsProps) {
      ...
      {toc.map(({ id, text, level }) => (
        ...
        >
          <ChevronRight className={cn(
            ...
            activeId === id ? "rotate-90 text-primary" : "text-muted-foreground/50 group-hover:text-muted-foreground"
          )} />
  ...
  if (isMobile) {
    return (
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-sm mx-4 max-h-[80vh] bg-gradient-to-br from-background via-background to-primary/5">
          <DialogHeader>
  ...
  // Desktop version
  return (
    <div className={cn("bg-gradient-to-br from-card via-card to-primary/5 border border-primary/10 rounded-xl p-6 shadow-sm", className)}>
